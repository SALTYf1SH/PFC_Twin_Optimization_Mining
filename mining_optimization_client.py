...
def run_simulation_worker(params_list, server, target_case_dir, job_id):
    ...
    sim_steps_json_string = load_from_knowledge_base(params_dict)
    ...
    if sim_steps_json_string is None:
        ...
        params_json_to_send = json.dumps(params_dict)
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                try:
                    ...
                    if "error" in response_data:
                        ...
                        save_to_knowledge_base(params_dict, sim_steps_json_string)
                except json.JSONDecodeError:
                    print(f"[Job {job_id}] FAILED: Could not parse JSON response from server.")
                    sim_steps_json_string = None
